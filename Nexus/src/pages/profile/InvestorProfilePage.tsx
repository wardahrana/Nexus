...
import {
  User, Building2, MapPin, DollarSign, Briefcase,
  Edit3, Save, X, Camera, Linkedin, Twitter,
  Github, CheckCircle, AlertCircle, Loader2, TrendingUp, BadgeDollarSign
} from 'lucide-react';
...
interface InvestorProfileData {
  ...
  firm?: string;
  designation?: string;
  investmentRange?: string | { min?: number; max?: number };
  preferredStages?: string[];
  preferredIndustries?: string[];
  investmentHistory?: string;
  preferences?: string | { lookingFor?: string }; 
  socialLinks?: SocialLinks;
}

type TabId = 'overview' | 'investments' | 'edit';
...
const Field: React.FC<{ label: string; value?: string; icon?: React.ReactNode; placeholder?: string }> = ({
...
// ── Helper: investmentRange object → string ──
const getInvestmentRange = (range: InvestorProfileData['investmentRange']): string => {
  if (!range) return '';
...
const getPreferencesString = (prefs: InvestorProfileData['preferences']): string => {
  if (!prefs) return '';
  if (typeof prefs === 'string') return prefs;
  return (prefs as { lookingFor?: string }).lookingFor || '';
};

export const InvestorProfilePage: React.FC = () => {
  const { user } = useAuth();
  const fileRef = useRef<HTMLInputElement>(null);

  const [activeTab, setActiveTab] = useState<TabId>('overview');
  const [profile, setProfile] = useState<InvestorProfileData>({});
  const [form, setForm] = useState<InvestorProfileData>({});
  ...
  const handleChange = (field: keyof InvestorProfileData, value: string) => {
  ...
  const handleTagChange = (field: 'preferredStages' | 'preferredIndustries', value: string[]) => {
    setForm(prev => ({ ...prev, [field]: value }));
  };
  ...
  const handleSave = async () => {
    ...
    const payload = {
  ...form,
  investmentRange: getInvestmentRange(form.investmentRange) || form.investmentRange,

  preferences: { lookingFor: typeof form.preferences === 'string' ? form.preferences : '' },
};
  ...
  const tabs: { id: TabId; label: string }[] = [
    ...
    { id: 'investments', label: 'Investment Profile' },
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6 animate-fade-in pb-12">
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
        <div className="h-28 bg-gradient-to-r from-accent-600 via-primary-600 to-primary-500" />
        <div className="px-6 pb-6">
          <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-4 -mt-12">
              ...
              <div className="w-24 h-24 rounded-2xl border-4 border-white shadow-md overflow-hidden bg-gray-100">
                {avatarPreview ? (
                  ...
                  <div className="w-full h-full flex items-center justify-center bg-accent-50">
                    <User size={36} className="text-accent-400" />
                  </div>
                )}
              </div>
              {activeTab === 'edit' && (
            ...
            <div className="flex-1 sm:ml-4 sm:mb-2">
              ...
              <p className="text-sm text-gray-500 flex items-center gap-1 mt-0.5 flex-wrap">
                <Building2 size={14} />
                {profile.firm ? `${profile.designation || 'Investor'} at ${profile.firm}` : 'Investor'}
                {profile.location && (
      ...
      {activeTab === 'overview' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
          <div className="md:col-span-2 bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            ...
            <p className={profile.bio ? 'text-gray-700 leading-relaxed' : 'text-gray-400 italic text-sm'}>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
            ...
            <Field label="Firm" value={profile.firm} icon={<Building2 size={15} />} />
            <Field label="Designation" value={profile.designation} icon={<Briefcase size={15} />} />
            <Field label="Location" value={profile.location} icon={<MapPin size={15} />} />
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-4">
            <h2 className="text-base font-semibold text-gray-900">Investment Info</h2>
            {/* ✅ Fix 3 — helper function use */}
            <Field label="Investment Range" value={getInvestmentRange(profile.investmentRange)} icon={<BadgeDollarSign size={15} />} />

            <div>
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Preferred Stages</span>
              <div className="flex flex-wrap gap-2 mt-2">
                {profile.preferredStages?.length
                  ? profile.preferredStages.map(s => (
                      <span key={s} className="px-3 py-1 bg-primary-50 text-primary-700 text-xs font-medium rounded-lg">{s}</span>
                    ))
            ...
            <div>
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider">Preferred Industries</span>
              <div className="flex flex-wrap gap-2 mt-2">
                {profile.preferredIndustries?.length
                  ? profile.preferredIndustries.map(i => (
                      <span key={i} className="px-3 py-1 bg-accent-50 text-accent-700 text-xs font-medium rounded-lg">{i}</span>
                    ))
                  : <span className="text-gray-400 italic text-sm">Not set</span>
                }
              </div>
      ...
      {activeTab === 'investments' && (
        <div className="space-y-5">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 space-y-5">
            <h2 className="text-base font-semibold text-gray-900">Investment Focus</h2>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {/* ✅ Fix 3 — helper function use */}
              <Field label="Investment Range" value={getInvestmentRange(profile.investmentRange)} icon={<DollarSign size={15} />} />
              
              <Field label="Firm" value={profile.firm} icon={<Building2 size={15} />} />
            </div>

            <div>
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Preferred Stages</span>
              <div className="flex flex-wrap gap-2">
                {profile.preferredStages?.length
                  ? profile.preferredStages.map(s => (
                      <span key={s} className="px-3 py-1.5 bg-primary-50 text-primary-700 text-sm font-medium rounded-lg border border-primary-100">{s}</span>
                    ))
                  : <span className="text-gray-400 italic text-sm">Not set</span>
                }
              </div>
            ...
            <div>
              <span className="text-xs font-semibold text-gray-400 uppercase tracking-wider block mb-2">Preferred Industries</span>
              <div className="flex flex-wrap gap-2">
                {profile.preferredIndustries?.length
                  ? profile.preferredIndustries.map(i => (
                      <span key={i} className="px-3 py-1.5 bg-accent-50 text-accent-700 text-sm font-medium rounded-lg border border-accent-100">{i}</span>
                    ))
                  : <span className="text-gray-400 italic text-sm">Not set</span>
                }
              </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-3 flex items-center gap-2">
              <TrendingUp size={17} className="text-primary-500" /> Investment History
            </h2>
            <p className={profile.investmentHistory ? 'text-gray-700 leading-relaxed' : 'text-gray-400 italic text-sm'}>
              {profile.investmentHistory || 'No investment history added yet.'}
            </p>
          </div>
      ...
      {activeTab === 'edit' && (
        <div className="space-y-5">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="md:col-span-2">
                ...
                <textarea
                  ...
                  placeholder="Tell entrepreneurs about your investment philosophy..."
              ...
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Firm</label>
                <input
                  type="text"
                  value={form.firm || ''}
          ...
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h2 className="text-base font-semibold text-gray-900 mb-5">Investment Details</h2>
            <div className="space-y-5">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Investment Range</label>
                {/* ✅ Fix 4 — helper function use in edit tab */}
                <input
                  type="text"
                  value={getInvestmentRange(form.investmentRange)}
              ...
              <TagSelect
                ...
                onChange={val => handleTagChange('preferredStages', val)}
              />
              ...
              <TagSelect
                ...
                onChange={val => handleTagChange('preferredIndustries', val)}
              />

              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1.5">Investment History</label>
                <textarea
                  rows={3}
                  value={form.investmentHistory || ''}
                  onChange={e => handleChange('investmentHistory', e.target.value)}
                  placeholder="Brief history of your past investments..."
                  className="w-full px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-primary-400 resize-none"
                />
              </div>
              ...
              <textarea
  rows={2}
  value={typeof form.preferences === 'string'
    ? form.preferences
    : (form.preferences as unknown as { lookingFor?: string })?.lookingFor || ''}
